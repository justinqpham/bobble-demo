...
class App extends React.Component<Props, State> {
  ...
  private remoteMoods_ = {"A_Cheering":0,"A_Booing":0,"A_BuildUp":0,"A_LetDown":0,"A_Clapping":0,"B_Cheering":0,"B_Booing":0,"B_BuildUp":0,"B_LetDown":0,"B_Clapping":0,"Neutral":1}
  ...
  componentWillMount() {
    ...
    Manifest.Context.map(manifest, fetch).then(loaded => {
  ...
  componentDidMount(){
    if(typeof(Storage) !== undefined && localStorage.getItem("bobble_id") !== null){
  ...
  private onStart_ = async (buffer, finalize, context) => {
    ...
    this.setState({ started: true, crowd: true }, async () => {
      try {
        ...
        const audioManifest = await Manifest.Context.map(this.loadedManifest_, buffer => new Promise<AudioBuffer>((resolve, reject) => {
        ...
        const resampledManifest = await Manifest.Context.map(audioManifest, buffer => Promise.resolve(resample(this.context_, buffer, this.context_.sampleRate)));
  ...
  private sendCheer_ = (mode: Mode) => async () => {
    ...
    return send(mode, this.state.sessionId);
  }
    ...
    return (
      <div className="App">
        ...
        <Phone trigger={this.renderStart} squish={this.state.squish}>
          <Start context_={this.context_} onStart_={this.onStart_} change={this.forward} display={this.state.start} />
          <Game emote={this.emote_} tog={this.tog} mood={this.remoteMoods_} display={this.state.game} state={this.state} back={this.back}/>
        </Phone>
      </div>
    );
  }
